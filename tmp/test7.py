...
def kmerize(string: str, k: int, modifier=None):
    if modifier:
...
def compare_kmer_sets(kmers1: set, kmers2: set):
...
def calculate_kmer_similarity(seq1: str, seq2: str, k: int):
...
def map_sequences(long_seq: str, short_seq: str, k: int):
    ...
    for i in range(len(long_seq) - len(short_seq) + 1):
        ...
        if similarity > best_similarity:
            ...
            best_position = f'{i + 1}..{i + 1 + len(short_seq)}'
    return best_position, best_similarity, best_coverage
