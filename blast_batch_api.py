...
def initiate_blast_search(sequences: str, database: str, service: str) -> Tuple[str, int]:
    url = "https://blast.ncbi.nlm.nih.gov/blast/Blast.cgi"
    query = quote(sequences)
    data = {
        "CMD": "Put",
        "PROGRAM": service,
        "DATABASE": database,
...
def perform_blast_batch(sequences: List[Seq], database: str, service: str) -> Tuple[str, int, List[str]]:
    concatenated_sequence = "\n".join(f">{seq.id}\n{seq.sequence}" for seq in sequences)
    rid, rtoe = initiate_blast_search(concatenated_sequence, database, service)

    time.sleep(rtoe)

    if poll_blast_results(rid):
        results = retrieve_blast_results(rid)
        return rid, rtoe, results.split('</BlastOutput_iterations>')[:-1]  # Split results by sequences
    else:
        logging.warning(f"No hits found for batch starting with sequence {sequences[0].id}")
        return rid, rtoe, [None for _ in sequences]
...
def main():
    ...
    with open(args.output_file, 'w', newline='') as tsvfile:
        ...
        writer.writerow(['Query ID', 'Hit ID', 'Hit Definition', 'Score', 'E-Value', 'Percent Identity'])
        results = set()

        BATCH_SIZE = 50
        for i in range(0, len(deduped_seqs), BATCH_SIZE):
            batch = deduped_seqs[i:i+BATCH_SIZE]
            rid, rtoe, batch_results = perform_blast_batch(batch, args.database, args.service)
            time.sleep(rtoe)

            if poll_blast_results(rid):
                raw_result = retrieve_blast_results(rid)

                for result in batch_results:
                    if result:
                        root = ET.fromstring(result)
                        query_id = root.find(".//Iteration_query-def").text
                        for hit in root.findall(".//Hit"):
                            ...
                            formatted_result = f"{query_id}\t{hit_id}\t{hit_def}\t{hit_score}\t{hit_evalue}\t{percent_identity:.2f}%"
